...
@Component({
  selector: 'ignatica-mjdkyidf',
  templateUrl: './mjdkyidf.component.html',
  styleUrls: ['./mjdkyidf.component.scss'],
})
export class MjdkyidfComponent implements OnInit {
  ...
  ngOnInit() {
    //this.disableControl = this.action == 'new';
  }
  ...
  savePlan(action: string) {
    ...
    if (
      this.planRenewalFormulaComponent &&
      this.planRenewalFormulaComponent.selectedRenewalFormulaObj
    ) {
      this.plan.generalConfiguration.planModules.push(
        this.planRenewalFormulaComponent.selectedRenewalFormulaObj
      );
  ...
  publishPlan(action: string) {
    ...
    let tableCoi = this.plan.feesChargesConfiguration.premiumOptions.filter(
      (x) => x.tableName == 'premium-coi-term-life'
    );
    if (
