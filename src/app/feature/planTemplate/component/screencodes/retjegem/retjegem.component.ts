...
@Component({
  selector: 'ignatica-retjegem',
  templateUrl: './retjegem.component.html',
  styleUrls: ['./retjegem.component.scss'],
})
export class RetjegemComponent implements OnInit {
  ...
  deductibleFeesChargesOID: string;
  product: ProductTemplate;
  @ViewChild(PlanRenewalFormulaComponent)
  planRenewalFormulaComponent: PlanRenewalFormulaComponent;
  @ViewChild(PlanMedicalBenefitsComponent)
  planMedicalBenefitsComponent: PlanMedicalBenefitsComponent;
  ...
  constructor(
    ...
    this.route.params.subscribe(({ action, id }) => {
      ...
      if (action == 'new') {
        ...
        this.plan.benefitConfiguration.medicalBenefitOptions =
          new MedicalBenefitsObject();
        this.plan.benefitConfiguration.medicalBenefitOptions.medicalBenefits =
          [];
      }
    ...
    if (!this.planService.productSelected) {
      ...
      this.deductibleFeesChargesOID = this.product.feesAndChargesOptions.filter(
        (x) =>
          x.feesChargesBasisId == 'IG_CALCULATION_BASIS_FIXED_AMOUNT' &&
          x.feesChargesConsumptionId == 'IG_CHARGE_DEDUCTION_TYPE_FORWARD' &&
          x.feesChargesTypeId == 'IG_FEES_CLAIM_DEDUCTIBLE'
      )[0].feesChargesOID;
    }
  }
  ...
  savePlan(action: string) {
    ...
    if (
      ...
      //  planCoiValid &&
      billingCompValid &&
      this.planMedicalBenefitsComponent.formValid &&
      underwritingValid
    ) {
      this.plan.generalConfiguration.planModules = [];
  ...
  publishPlan(action: string) {
    ...
    if (this.planMedicalBenefitsComponent.medicalBenefits.length <= 0) {
      const message = this.translateService.instant(
        'plan.medical.benefit.benefitsRequired'
      );
      this.toastService.show({
